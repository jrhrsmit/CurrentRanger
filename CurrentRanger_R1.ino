...
//***********************************************************************************************************
#define LDO_OUTPUT \
    3.307  // volts, change to actual LDO output (measure GND-3V on OLED header)
//***********************************************************************************************************
...
#define ADC_OVERLOAD \
    4000  // assuming DAC output is very close to 0, this is max value less
          // ground offset (varies from unit to unit, 4000 is a safe value)
//***********************************************************************************************************
#define ADC_CALIBRATE_EN
//#define ADC_CALIBRATE_FORCED       //uncomment to set manual offset/gain
// values below
...
//***********************************************************************************************************
#define SERIALBAUD 1382400  // Serial baud for HC-06 bluetooth output
...
*/
    ...
    // DAC->CTRLB.bit.REFSEL=0;//pick internal reference, skip SYNCDAC (done by
    // analogWrite)
...
#else
                                          // or hardcoded:
    analogReadCorrectionForced(ADC_CALIBRATE_FORCED_OFFSET,
                               ADC_CALIBRATE_FORCED_GAIN);
    //(offset, gain) - gain is 12 bit number (1 bit integer + 11bit fractional,
    // see DS p895)
    ...
    SerialUSB.print("Bluetooth AT check @");
    SerialUSB.print(SERIALBAUD);
    ...
    {
        if (Serial.available() == 2 && Serial.read() == 'O' &&
            Serial.read() == 'K') {
            BT_found = true;
            break;
        }
    ...
    SerialUSB.print(BT_found ? "OK!"
                             : "No response. Checking for version 3.0.\r\n");
    ...
    if (!BT_found) {
        ...
        {
            if (Serial.available() == 4 && Serial.read() == 'O' &&
                Serial.read() == 'K' && Serial.read() == '\r' &&
                Serial.read() == '\n') {
                BT_found = true;
                break;
            }
    ...
    /*
      // ADC Linearity/Bias Calibration from NVM (should already be done done in
      core) uint32_t bias = (*((uint32_t *) ADC_FUSES_BIASCAL_ADDR) &
      ADC_FUSES_BIASCAL_Msk) >> ADC_FUSES_BIASCAL_Pos; uint32_t linearity =
      (*((uint32_t *) ADC_FUSES_LINEARITY_0_ADDR) & ADC_FUSES_LINEARITY_0_Msk)
      >> ADC_FUSES_LINEARITY_0_Pos; linearity |= ((*((uint32_t *)
      ADC_FUSES_LINEARITY_1_ADDR) & ADC_FUSES_LINEARITY_1_Msk) >>
      ADC_FUSES_LINEARITY_1_Pos) << 5; ADC->CALIB.reg = ADC_CALIB_BIAS_CAL(bias)
...
uint32_t oledInterval = 0, lpfInterval = 0, offsetInterval = 0,
         autorangeInterval = 0, btInterval = 0;
...
void handleTouchPads() {
    ...
    // range switching
    if (!AUTORANGE) {
        if (MA_PRESSED && UA_NOT_PRESSED && NA_NOT_PRESSED &&
            rangeUnit != 'm') {
            rangeMA();
            rangeBeep();
        }
        if (UA_PRESSED && MA_NOT_PRESSED && NA_NOT_PRESSED &&
            rangeUnit != 'u') {
            rangeUA();
            rangeBeep();
        }
        if (NA_PRESSED && UA_NOT_PRESSED && MA_NOT_PRESSED &&
            rangeUnit != 'n') {
            rangeNA();
            rangeBeep();
        }
    ...
    if (UA_PRESSED && NA_PRESSED && MA_NOT_PRESSED &&
        millis() - lpfInterval > 1000) {
        toggleLPF();
        Beep(3, false);
    }

    // offset toggling (GNDISO to half supply) --- [MA+UA]
    if (MA_PRESSED && UA_PRESSED && NA_NOT_PRESSED &&
        millis() - offsetInterval > 1000) {
        toggleOffset();
        Beep(3, false);
    }

    // AUTORANGE toggling
    if (MA_PRESSED && NA_PRESSED && UA_NOT_PRESSED &&
        millis() - autorangeInterval > 1000) {
        toggleAutoranging();
        Beep(20, false);
        delay(50);
        (20, false);
...
void readVOUT() {
    ...
    for (byte i = 0; i < ADCREADINGS; i++) {
        ...
        // readDiff+=analogDifferentialRaw(0x1C, 0x04); //DAC-AIN4
        // (mux_pos,uint8_t mux_neg) readDiff+=analogDifferentialRaw(0x04,
        // 0x03);
        // //AIN4-AIN3 readDiff+=analogDifferential(SENSE_OUTPUT,SENSE_GNDISO);
    }
...
void toggleAutoranging() {
    ...
    AUTORANGE = !AUTORANGE;
    if (AUTORANGE && OFFSET)
        toggleOffset();  // turn off OFFSET
    if (AUTORANGE && !LPF)
        toggleLPF();  // turn on OFFSET
}
...
int adcRead(byte ADCpin) {
    ...
    // // take 1 sample, adjusting result by 0 ADC->AVGCTRL.reg =
    // ADC_AVGCTRL_SAMPLENUM_16 | ADC_AVGCTRL_ADJRES(0x4ul); //take 16 samples
    // adjust by 4
    ADC->AVGCTRL.reg =
        ADC_AVGCTRL_SAMPLENUM_1024 |
        ADC_AVGCTRL_ADJRES(0x4ul);  // take 1024 samples adjust by 4
...
//***********************************************************************************************************
// ADC OFFSET/GAIN CALIBRATION - adapted from
// SAMD_AnalogCorrection>CorrectADCResponse Example
// calibration runs automatically ONCE after unit is (re)programmed, and stores
// calib values in EEPROM (EEPROM is emulated and gets erased when reflashing
// SAMD21)
//***********************************************************************************************************
...
void calibrateADC() {
    ...
    if (highLevelRead < ADC_TOP_VALUE) {
        ...
        for (uint16_t gain = ADC_UNITY_GAIN - 1; gain >= ADC_MIN_GAIN; --gain) {
            ...
            SerialUSB.print("* LOOP : minGain=");
            SerialUSB.print(minGain);
            SerialUSB.print(" maxGain=");
            SerialUSB.println(maxGain);
            if (highLevelRead < ADC_TOP_VALUE) {
                if (minGain == 0U)
                    minGain = maxGain;
                break;
...
void WDTset() {
    ...
    // Enable clock generator 2 using low-power 32KHz oscillator. With /32
    // divisor above, this yields 1024Hz(ish) clock.
